    ...
    return next.handle(intReq).pipe(catchError((err: HttpErrorResponse) => {
      if(err.status === 401){
          ...
          return this.authService.refresh(dto).pipe(concatMap((data:any) =>{
            ...
            return next.handle(intReq) as Observable<HttpEvent<unknown>>;
          }))
      }else{
    ...
    return next.handle(intReq).pipe(
      catchError((err: HttpErrorResponse) => {
        if (err.status === 401) {
          ...
          ) as Observable<HttpEvent<unknown>>; // Especificando o tipo de retorno
        } else {
          this.tokenService.logOut();
          return throwError(err) as Observable<HttpEvent<unknown>>; // Especificando o tipo de retorno
        }
      })
    );
  }*/
